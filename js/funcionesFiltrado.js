export const pasarPagina = async () => {
    ...
    const productos = await obtenerProductos(params.get("categoria"), filtros)
    const etiquetas = rellenarCatalogo(productos)
    mostrarFiltros(etiquetas)
    mostrarOrdenarPor(productos)
    return etiquetas
  }
  
  const obtenerProductos = async (categoria, filtros) => {
    ...
    for (let index = 0; index < data.length; index++) {
      if (data[index].categoria === categoria) {
        ...
        for (const claveFiltro in filtros) {
          ...
          if (claveFiltro === "tallas") valorProducto = Object.keys(producto.cantidades || {})
          if (claveFiltro === "color") valorProducto = producto.elementos[0].color
  ...
  const mostrarOrdenarPor = (informacionProductos) => {
    ...
    document.getElementById("mejorDescuento").addEventListener("click", () => {
      informacionProductos.sort((a, b) => parseFloat(b.descuento || 0) - parseFloat(a.descuento || 0))
      ...
      if (!filtrosAbiertos) {
        ...
        for (let index = 0; index < Object.keys(filtros).length; index++) {
          ...
          filtros[Object.keys(filtros)[index]].forEach(elementos => {
            ...
            textoBoton.textContent = elementos
        ...
        botones.forEach(boton => {
          boton.addEventListener("click", () => {
            ...
            const filtroNombre = divFiltro.querySelector("h2").textContent.trim()
